...
static int create_dir_entry_cache(const char *path, uint32_t parent_cluster, uint32_t *allocated_cluster)
{
	...
	while (true)
	{
		...
		if (finfo.type == LFS_TYPE_DIR && parent_cluster == 0 && (strcmp(finfo.name, ".") == 0 || strcmp(finfo.name, "..") == 0))
		{
			continue;
		}
...
static void restore_file_from(char *result_filename, uint32_t directory_cluster_id, uint32_t file_cluster_id)
{
	...
	while (cluster_id >= 0)
	{
		...
		for (int i = 0; i < 16; i++)
		{
			...
			else if (dir[i].DIR_Attr & 0x20 || dir[i].DIR_Attr == 0x00)
			{ // is file
				if (is_long_filename)
				{
					utf16le_to_utf8(filename, sizeof(filename), long_filename, sizeof(long_filename));
									...
									fat_dir_entry_t *delete)
{
	...
	for (int i = 0; i < 16; i++)
	{
		if (strncmp((const char *)new[i].DIR_Name, ".          ", 11) == 0 || strncmp((const char *)new[i].DIR_Name, "..         ", 11) == 0 || (new[i].DIR_Attr & 0x0F) == 0x0F || (new[i].DIR_Attr & 0x08) == 0x08) // volume label
		{
			continue;
		}
		...
		if (new[i].DIR_Name[0] == 0xE5)
		{
			for (int j = 0; j < 16; j++)
			{
				if ((orig[j].DIR_Attr & 0x08) == 0x08) // volume label
					continue;
				if (new[i].DIR_FstClusLO == orig[j].DIR_FstClusLO &&new[i].DIR_FileSize == orig[j].DIR_FileSize && orig[j].DIR_Name[0] != 0xE5 && new[i].DIR_FileSize != 0)
				{
					// `delete` or `rename`.
				...
				if (orig[j].DIR_Attr & 0x10 // directory
					&& new[i].DIR_FstClusLO == orig[j].DIR_FstClusLO &&new[i].DIR_FileSize == 0 && orig[j].DIR_Name[0] != 0xE5 && i == j)
				{
					// `delete` or `rename`.
					memcpy(delete, &orig[j], sizeof(fat_dir_entry_t));
					delete ++;
		...
		for (int j = 0; j < 16; j++)
		{
			if (new[i].DIR_Name[0] == 0xE5 || strncmp((const char *)orig[j].DIR_Name, ".          ", 11) == 0 || strncmp((const char *)orig[j].DIR_Name, "..         ", 11) == 0 || (orig[j].DIR_Attr & 0x0F) == 0x0F || (orig[j].DIR_Attr & 0x08) == 0x08) // volume label
			{
				continue;
			}

			if (strncmp((const char *)new[i].DIR_Name, (const char *)orig[j].DIR_Name, 11) == 0 &&
				new[i].DIR_FstClusLO == orig[j].DIR_FstClusLO &&new[i].DIR_FileSize == orig[j].DIR_FileSize)
			{
				is_found = true;
				break;
			}
...
static void update_lfs_file_or_directory(fat_dir_entry_t *src, uint32_t dir_cluster_id)
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		else if (dir->DIR_Attr & 0x20 || dir->DIR_Attr == 0x00)
		{ // is file
			if (is_long_filename)
			{
				utf16le_to_utf8(filename, sizeof(filename), long_filename, sizeof(long_filename));
...
void mimic_fat_write(uint8_t lun, uint32_t request_block, void *buffer, uint32_t bufsize)
{
	...
	if (is_fat_sector(request_block))
	{ // FAT table
