...
namespace PicoMill::Drivers
{
	...
	PIOStepper::PIOStepper(uint astepPin, uint adirPin, uint aenPin, uint32_t maxSpeed, uint32_t acceleration, uint32_t decelerationMultiplier, /*PIO pio, uint sm,*/ uint16_t astepsPerRev)
		: stepPin(astepPin), dirPin(adirPin), enPin(aenPin), myAcceleration(acceleration), myDecelerationMultiplier(decelerationMultiplier), /* pio(pio), sm(sm),*/ myStepsPerRev(astepsPerRev) //, IStepper(maxSpeed, acceleration)
	{
		// gpio_init(dirPin);
	...
	void PIOStepper::Update()
	{
		...
		uint32_t delay = floor(1.0 / ((myCurrentSpeed ? myCurrentSpeed : ACCELERATION_JERK) / 1000000.0));
