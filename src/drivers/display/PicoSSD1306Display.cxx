...
#define SSD1306_BUF_LEN (SSD1306_NUM_PAGES * SSD1306_WIDTH)
		  ...
		  myBufferLen(SSD1306_BUF_LEN)
	{
		...
		printf("PicoSSD1306Display: Display address 0x%02x\n", myDisplayAddress);
		...
		// Create a task to initialize the display
		xTaskCreate(InitTask, "SSD1306_Init", configMINIMAL_STACK_SIZE * 4, this, tskIDLE_PRIORITY + 1, nullptr);
	}
	...
	void PicoSSD1306Display::InitTask(void *pvParameters)
	{
		vTaskDelay(pdMS_TO_TICKS(1)); // Wait for I2C to settle
		auto instance = static_cast<PicoSSD1306Display *>(pvParameters);

		// Check if display is present on I2C bus using a timeout
		if (!instance->CheckDeviceResponsive())
		{
			printf("PicoSSD1306Display: Display not detected on I2C bus, address 0x%02x\n", instance->myDisplayAddress);
			instance->myIsReady = false;
			ExitTask();
		}

		// Allocate frame buffer
		instance->myFrameBuffer = new uint8_t[instance->myBufferLen];
		if (instance->myFrameBuffer == nullptr)
		{
			printf("PicoSSD1306Display: Failed to allocate frame buffer\n");
			ExitTask();
		}

		// Clear buffer
		memset(instance->myFrameBuffer, 0, instance->myBufferLen);

		// Initialize display
		if (!instance->InitDisplay())
		{
	...
	static inline int GetFontIndex(uint8_t ch)
	{
		...
		else if (ch >= '0' && ch <= '9')
		{
			...
			return 0; // Not supported, use space
	}
